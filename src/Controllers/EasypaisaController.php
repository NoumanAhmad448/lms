...
class EasypaisaController extends Controller
{
    ...
    public function getEasypay()
    {
        ...
        // dd($timestamp);
        return view('lms::easypaisa.send-request-easypaisa',compact('time'));
    ...
    public function getJazz(){
        return view('lms::jaxxcash/jaxxcash');
    }
    ...
    public function getStarted($course)
    {
        try{
            ...
            $timestamp = strtotime(date("h:i:sa")) + 2*60*60;
            $time = date('Y-m-d h:i:s', $timestamp);
            return view('lms::easypaisa.send-request-easypaisa',compact('price','time','slug'));
        }
        catch(\Throwable $e){
            return back();
        }
