...
class DController extends Controller
{
    ...
    public function show($course_id)
    {
        try {
            ...
            return view('lms::courses.dashboard_instructor', compact('course', 'course_id', 'title'));
        } catch (Exception $e) {
    ...
    {
        $course = $this->validate_user($course_id);
        return view('lms::courses.course_structure', compact('course'));
    }

    public function course_setup($course_id)
    {
        $course = $this->validate_user($course_id);
        return view('lms::courses.course_setup', compact('course'));
    }

    public function course_film($course_id)
    {
        $course = $this->validate_user($course_id);
        return view('lms::courses.courses_film_edit', compact('course'));
    }
    ...
    public function course_curriculum($course_id)
    {
        ...
        $section = Section::where('course_id', $course_id)
            ->get();
            // dd($section);
        return view('lms::courses.course_curriculum', compact('course', 'section'));
    }
    ...
    public function course_curriculum_post(Request $request, $course_id)
    {
        ...
        if ($request->ajax()) {
            ...
            $rules = array('sec_title' => 'required|max:255', 'sec_no' => 'required|integer');
