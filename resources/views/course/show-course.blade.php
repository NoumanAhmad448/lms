                        ...
                        @if ($rating_avg)
                            <div class="d-flex align-items-center">
                                <section id="rating" class="d-flex align-items-center" style="cursor: pointer">
                                    ...
                                    <span class="fa fa-2x fa-star ml-1 rating" style="text-size: 1.3rem;"
                                        no="2"></span>
                                    <span class="fa fa-2x fa-star ml-1 rating" style="text-size: 1.3rem;"
                                        no="3"></span>
                                    <span class="fa fa-2x fa-star ml-1 rating" style="text-size: 1.3rem;"
                        ...
                        <div class="mt-2">
                            <section class="d-flex">
                                @if (config('setting.course_desc_wishlist_btn'))
                                    <form action="{{ route('wishlist-course-post', ['slug' => $course->slug]) }}"
                                        method="POST">
                                        @csrf
                                ...
                                @if (config('setting.course_desc_gift_btn'))
                                    <a href="" class="btn btn-light ml-2"> <i class="fa fa-gift"
                                            aria-hidden="true"></i>
                                        Gift Course
                                    </a>
                                @endif
        ...
        <div class="border">
            ...
            <div class="px-2">
                ...
                @auth
                    @php
                        $u_id = auth()->id();
                        $enrolled_s = CourseEnrollment::where('user_id', $u_id)
                        ...
                        @if ($course->lecture)
                            @if ($course->lecture->media)
                                @if ($course->lecture && $course->lecture->media && $course->lecture->media->lec_name)
                                    @if (allowCourseToAdmin() || auth()->id() == $course->user_id || ($enrolled_s && $enrolled_s->count()))
    ...
    <div class="container mt-3">
                                    ...
                                    @foreach ($sections as $sec)
                                        <div class="card-header" id="headingOne">
                                            <div class="d-flex justify-content-between align-items-center">
                                                <h2 class="mb-0">
                                                    <button
                                                        class="btn btn-link btn-block text-left font-bold text-dark text-capitalize"
                                        ...
                                        <div id="section{{ $sec->id }}"
                                            class="collapse show @if ($sec->section_no == 1)  @endif"
        ...
        margin-top: 2rem;">
                ...
                <div class="d-flex align-items-center">
                    <section id="rating" class="d-flex align-items-center" style="cursor: pointer">
                        <span
                            class="fa fa-2x fa-star       @if ($rating_avg >= 1) {{ 'text-warning' }} @endif"
                            no="1"></span>
                        <span
                            class="fa fa-2x fa-star ml-1  @if ($rating_avg >= 2) {{ 'text-warning' }} @endif"
                            style="text-size: 1.3rem;" no="2"></span>
                        <span
                            class="fa fa-2x fa-star ml-1  @if ($rating_avg >= 3) {{ 'text-warning' }} @endif"
                            style="text-size: 1.3rem;" no="3"></span>
                        <span
                            class="fa fa-2x fa-star ml-1  @if ($rating_avg >= 4) {{ 'text-warning' }} @endif"
                            style="text-size: 1.3rem;" no="4"></span>
                        <span
                            class="fa fa-2x fa-star ml-1  @if ($rating_avg >= 5) {{ 'text-warning' }} @endif"
        ...
        @if (isset($comments) && count($comments) > 0)
            ...
            @foreach ($comments as $c)
                ...
                <section class="row my-3">
                    <div class="col-2">
                        <img height="50" width="50" class="rounded-circle object-cover"
                            src="@if ($user && $user->profile_photo_path) {{ asset($user->profile_photo_path) }} @else
                                {{ $user ? $user->profile_photo_url : '' }} @endif"
                            alt="{{ $user->name ?? '' }}" />
                    </div>
                    <div class="col">
                        ...
                        @php
                            if (isset($c)) {
                                $rate = $c->rating ? $c->rating->rating : 0;
                            }
                        @endphp
                        @if (isset($rate) && $rate > 0)
                            <div class="d-flex align-items-center">
                                <section id="rating" class="d-flex align-items-center" style="cursor: pointer">
                                    <span
                                        class="fa fa-star       @if ($rate >= 1) {{ 'text-warning' }} @endif"
                                        no="1"></span>
                                    <span
                                        class="fa fa-star ml-1  @if ($rate >= 2) {{ 'text-warning' }} @endif"
                                        style="text-size: 1.3rem;" no="2"></span>
                                    <span
                                        class="fa fa-star ml-1  @if ($rate >= 3) {{ 'text-warning' }} @endif"
                                        style="text-size: 1.3rem;" no="3"></span>
                                    <span
                                        class="fa fa-star ml-1  @if ($rate >= 4) {{ 'text-warning' }} @endif"
                                        style="text-size: 1.3rem;" no="4"></span>
                                    <span
                                        class="fa fa-star ml-1  @if ($rate >= 5) {{ 'text-warning' }} @endif"
                                        style="text-size: 1.3rem;" no="5"></span>
                                </section>
                            </div>
                        @endif
    ...
    <div class="col-md-4 d-none d-md-block">
        <div class="border p-1">
            ...
            @if ($c_vid && $c_vid->vid_path && config('setting.show_course_video'))
                <media-player src="{{ config('setting.s3Url') }}{{ $c_vid->vid_path }}" aspect-ratio="16/9"
                    id="vid01">
                    <media-outlet>
            ...
            <div class="px-2">
                ...
                @auth
                    @if (config('setting.show_hide_coupon') &&
                            (!($enrolled_s && $enrolled_s->count()) && !allowCourseToAdmin()) &&
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body" id="share_m_body">
                    <div class="">
                        ...
                        <div class="d-flex justify-content-end">
                            <input type="hidden" id="link"
                                value="{{ route('user-course', ['slug' => $course->slug]) }}">
                    ...
                    &body={{ route('user-course', ['slug' => $course->slug]) }}"
                                class="btn btn-website btn-sm" target="_blank">
...
@section('script')
    ...
    <script src="{{ asset('vendor/lms/js/course/show-course.js') }}"></script>
@endsection
