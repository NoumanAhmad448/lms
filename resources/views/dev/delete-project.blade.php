...
@section('content')
<button class="delete-files-btn btn btn-primary" data-url="{{route("deleteProjectDelete")}}" data-path="/" onclick="confirmDelete(this)">
    Delete Files
</button>
...
function sendDeleteRequest(url, path) {
    $.ajax({
        url: url,
        type: 'POST',
        data: JSON.stringify({ path: path , _method: 'DELETE'}),
        ...
        error: function(xhr) {
            ...
            showResponse({ message: errorMsg, failed_deletes: [] });
        }
    });
}
