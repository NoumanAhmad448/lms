    ...
    <div class="container my-5">
                    ...
                    @if ($course->slug)
                        <div class="col-md-3">
                            ...
                            <a href="{{ route('user-course', ['slug' => $course->slug]) }}">
                                @if ($ci)
                                    <img class="card-img-top" src="{{ config('setting.s3Url') . $ci->image_path }}"
                                        alt="{{ $ci->image_name }}">
                                @endif
                            </a>
                        </div>
                        <div class="col-md-9 p-3">
                            ...
                            <p class="card-text text-capitalize mt-1"> @if ($course->price->is_free)  {{ __('lms::free') }}
                            ...
                            <div class="card-body" style="height: 150px">
                                <h5 class="card-title font-bold text-capitalize" style="font-size: 1.1rem;font-weight:bold">
                                    {{ reduceCharIfAv($course->course_title ?? '', 40) }} </h5>
                                <p class="card-text text-capitalize mb-0 mt-1"><span class="">
                                        @if (!empty($course->user))
                                            {{ reduceWithStripping($course->user->name, 20) ?? '' }}
                                        @endif
                                    </span>
                                </p>
                                <p
        ...
        @else
            <section class="d-flex justify-content-center flex-column align-items-center">
                <img src="{{ asset('vendor/lms/img/record.jpg') }}" alt="record" class="img-fluid w-50" />
