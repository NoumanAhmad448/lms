    ...
    <div class="shadow-lg">
        ...
        @if (file_exists(public_path('storage/uploads/' . $property->featured_image)))
            <div class="py-20 flex-1 bg-center"
                style="background-image: url('/storage/uploads/{{ $property->featured_image }}')"></div>
        @else
        ...
        <div class="p-3">
            ...
            <div class="border-t-2">
                <ul class="flex items-center -mx-1 my-4">
                    <li class="px-2 py-1 bg-gray-200 rounded-md text-xs mx-1 shadow-sm">{{ $property->bedrooms }}
                        {{ __('lms::Bedrooms') }}</li>
                    <li class="px-2 py-1 bg-gray-200 rounded-md text-xs mx-1 shadow-sm">{{ $property->bathrooms }}
                        {{ __('lms::Bathrooms') }}</li>
                    <li class="px-2 py-1 bg-gray-200 rounded-md text-xs mx-1 shadow-sm">{{ $property->gross_sqm }}
                        {{ __('lms::ft') }}<sup>2</sup></li>
                </ul>
                <a href="{{ route('property.show', $property->id) }}"
                    class="btn w-full text-center">{{ __('lms::More details') }}</a>
            </div>
        </div>
    </div>
</div>
