                ...
                <div class="jumbotron text-center">
                    ...
                    <form id="nextform"
                        ...
                        <select class="custom-select categories" name="categories_selection">
                            @if ($categories->count())
                                <option @if (!$course_category) {{ __('lms::selected') }} @endif
                                    value="Choose a category"> Choose a category </option>
                                @foreach ($categories as $c)
                                    @php $value = $c->value; @endphp
                                    <option @if ($course_category == $value) {{ __('lms::selected') }} @endif
                                        value="{{ $value ?? null }}"> {{ $c->name ?? null }} </option>
                                @endforeach
                            @endif
                        </select>
                    </form>
...
@section('course_footer')
    <script src="{{ asset('vendor/lms/js/course_category.js') }}"></script>
@endsection
