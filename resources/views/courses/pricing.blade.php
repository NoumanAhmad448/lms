            ...
            <form url="{{ route('pricing', compact('course')) }}" class="p_price">
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="number" min="1" max="500" class="form-control pricing" id="pricing"
                            name="pricing" placeholder="Set Course Price"
                            value="@if ($pricing && $price) {{ $price }} @endif"
                            @if (!$price && isset($pricing)) {{ __('lms::disabled') }} @endif>
                        <div id="pricing_err" class="text-danger"></div>
                    </div>
                </div>
                <div class="form-group form-check">
                    <input type="checkbox" class="form-check-input" id="free" name="free"
                        @if ($pricing && $free) {{ __('lms::checked') }} @endif>
                    <label class="form-check-label" for="free">Mark Free</label>
                </div>

                <button type="submit" class="btn btn-info"
                    @if ($price || $free) @else {{ __('lms::disabled') }} @endif><i
...
@section('page-js')
    <script src="{{ asset('vendor/lms/js/price.js') }}"></script>
@endsection
