...
@php
    ...
    use Eren\Lms\Models\LanguageModal;
            ...
            <form url="{{ route('landing_page_post', compact('course')) }}" class="landing_form">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="course_desc" rows="10" cols="10" name="course_desc"
                        title="Input your course description" placeholder="Course Description">{{ $course->description ?? '' }}</textarea>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <select class="js-example-responsive js-states form-control" id="select_level" name="select_level">
                            ...
                            <option value="beginner" @if ($c_level === 'beginner') {{ __('lms::selected') }} @endif>
                                Beginner
                            </option>
                            <option value="intermediate" @if ($c_level === 'intermediate') {{ __('lms::selected') }} @endif>
                                Intermediate </option>
                            <option value="advance" @if ($c_level === 'advance') {{ __('lms::selected') }} @endif>
                                Advance </option>
                            <option value="all_level" @if ($c_level === 'all_level') {{ __('lms::selected') }} @endif>
                                All level
                            </option>
                        </select>
                    ...
                    <div class="col-md-6 mt-3 mt-md-0">
                        <select class="js-example-responsive js-states form-control" id="select_category"
                            name="select_category">
                            <option value=""> Select Category </option>
                            @php $c_category = $course->categories_selection; @endphp
                            @if (isset($categories) && $categories)
                                @foreach ($categories as $c)
                                    <option value="{{ $c->value ?? null }}"
                                        @if ($c_category === $c->value) {{ __('lms::selected') }} @endif>
                                        {{ $c->name ?? null }} </option>
                                @endforeach
                            @endif
                        </select>
                                ...
                                @if (isset($langs) && $langs)
                                    @foreach ($langs as $lang)
                                        <option value="{{ $lang->id ?? null }}"
                                            @if (isset($course) && $course->lang_id === $lang->id) {{ __('lms::selected') }} @endif>
                ...
                <div class="col-md-6">
                    <section class="d-flex justify-content-center align-items-center flex-column h-100">
                        ...
                        <div class="text-info text-left w-100" style="  text-align: left !important "> Please upload a Image
                            of format jpeg,png,jpg,gif,tif with at most size 10MB</div>
        ...
        <section class="course_vid my-5">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    @else
                        <img src="{{ asset('vendor/lms/img/thumbnail.jpg') }}" alt="Course Thumbnail"
                            class="img-fluid video_img" width="750" height="450" />
                    @endif
                </div>
                    ...
                    <section class="d-flex justify-content-center align-items-center flex-column h-100">
                        ...
                        <section class="img_con mt-3 w-100">
                            ...
                            <div class="progress mt-3 vid_p_con d-none">
                                <div class="progress-bar vid_p_bar" style="width: 0%" role="progressbar"
...
@section('page-js')
    ...
    <script src="{{ asset('vendor/lms/js/landing_page.js') }}"></script>
    <script>
