                    ...
                    @if (config('setting.coupon_form'))
                        <section class="c_con mt-4">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="coupon_no" placeholder="Coupon"
                                    name="coupon_no">
                                <small class="form-text text-muted">write any specific word of your choice and share it with
                                    others
                        ...
                        <div class="container">
                            <div class="row">
                                <div class="col-12 col-md-10">
                                    <div class="form-group">
                                        <input type="text" class="form-control" id="coupon_no_{{ $p->id }}"
                                            placeholder="Coupon" name="coupon_no" value="{{ $p->coupon_code ?? '' }}">
                                    </div>
                                </div>
                                <div class="col-2 col-md-1">
                                    <div class="btn btn-danger del_coupon"
                                        url="{{ route('delete_coupon', ['promotion' => $p]) }}"> Delete </div>
                                </div>
                                <div class="col-6 ml-3 ml-md-0 col-md-1">
                                    <button type="button" class="btn btn-info update-coupon"
                                        ...
                                        coupon_no="{{ $p->coupon_code }}" id="{{ $p->id }}"
                                        date_time="{{ $p->date_time }}" is_free="{{ $p->is_free }}"
                                        no_of_coupons="{{ $p->no_of_coupons }}" percentage="{{ $p->percentage }}"> Update
                                    </button>
                                </div>
                            </div>
                        </div>
...
@section('page-js')
    ...
    <script src="{{ asset('vendor/lms/js/promotion.js') }}"></script>
@endsection
