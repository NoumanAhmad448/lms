<div>
        ...
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td> {{ $user->name ?? '' }} </td>
                    <td> {{ $user->email ?? '' }} </td>
                    @if(isSuperAdmin()) <td> <a href="{{route('edit_admin___',compact('user'))}}" > Edit </a> </td> @endif
                    @if(isSuperAdmin()) <td> <div class="cursor_pointer text-danger del-admin" link="{{route('delete_admin___',compact('user'))}}" > Delete </div> </td> @endif
                </tr>
                @endforeach
            ...
            <div class="modal-body">
                <section class="py-3">
                    <div> You are about to delete the blogger accout. This operation is irreversible which means 
                        you will not be able to login to this site using this profile. 
                    </div>
                    <div> 
    ...
    <script>
        $(function(){
            $('#show_sub_admins').addClass('bg-website text-white').removeClass('text-dark font-weight-bold');
            setTimeout(() => {
                $('.alert').fadeOut();
            }, 5000);
