<nav class="w-full py-2 px-12 flex justify-between items-center sticky top-0 z-50
     general-header">
    <div class="min-w-max">
        <a href="{{ route('index') }}"><img width="100" src="{{ url(config('setting.im_log')) }}"
    ...
    <div class="hidden w-full md:block md:w-auto" id="navbar-dropdown">
        <ul
            ...
            @if (route('land_create'))
                <li><a class="inline-block p-4 text-white {{ request('type') == '3' ? 'bg-gray-50' : '' }}"
                        href='{{ route('land_create') }}'>{{ __('lms::messages.reg_land') }}</a>
                </li>
            @endif
    ...
    <div class="min-w-max text-3xl flex justify-end">
        @include(config('files.components') . '.user_profile', [
            'prop' => ['logout_url' => route('logout')],
        ])
    </div>
    @if (!auth()->user())
        <div class="hidden w-full md:block md:w-auto">
            <a href="{{ route('login') }}">{{ __('lms::messages.login') }}</a>
        </div>
    @endif
