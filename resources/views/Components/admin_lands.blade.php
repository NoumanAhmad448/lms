        ...
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5 p-4">
            @include(config('files.forms') . 'three_col', [
                ...
                config('vars.prop') => [
                    ...
                    'include_star' => false,
                    'label' => __('lms::messages.land_op'),
                    'data' => __('lms::messages.lnd_oprtn'),
                ],
            ])
        ...
        <thead>
            <tr>
                ...
                @can(config('policy.is_super_admin'))
                    <th>{{ __('lms::table.' . config('table.land_logs')) }}</th>
                @endcan
                @can(config('policy.view_land_comment'))
                    <th>{{ __('lms::table.' . config('table.land_comments_logs')) }}</th>
                @endcan
                ...
                @foreach ($data['lands'] as $lands)
                    <tr>
                        ...
                        <td onclick="showMap('{{ $lands?->location }} {{ $lands?->city?->name }}','{{ $map }}')"
                            class="underline cursor-pointer" data-modal-target = "{{ $map }}">
                            {{ $lands?->location }}
                        </td>
                        <td>{{ $lands?->size }}</td>
...
$land_approval = $lands->landComment && count($lands->landComment);
                            ...
                            {{ $is_admin_approved ? __('lms::messages.yes') : __('lms::messages.no') }}
                        </td>
                        <td>{{ $is_admin_approved ? $lands->landComment[0]->comment : '' }}</td>
                        @can(config('policy.is_super_admin'))
                            <th>
                                <a target="_blank" class="hover:no-underline underline"
                                    href="{{ route('land_logs', [config('table.primary_key') => $lands->id]) }}">
                                    {{ __('lms::table.land_logs') }}
                                </a>
                            </th>
                        @endcan
                        @can(config('policy.view_land_comment'))
                            <th>
                                <a target="_blank" class="hover:no-underline underline"
                                    href="{{ route('comment_logs', [config('table.primary_key') => $lands->id]) }}">
                                    {{ __('lms::table.land_comments_logs') }}
                                </a>
                            </th>
                        @endcan
                        <td>{{ $is_admin_approved && $lands->landComment[0]->user ? $lands?->landComment[0]?->user?->name : '' }}
                        </td>
                        @if (config('setting.en_slf'))
                            <td>
