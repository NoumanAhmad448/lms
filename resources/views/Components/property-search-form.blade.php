<form action="{{route('property.index')}}" method="GET" class="flex justify-between" style="font-family: 'Nunito', sans-serif;">
    <div class="flex flex-1 justify-between items-center">
        <div class="flex flex-col mx-3">
            ...
            <select id="sale" name="sale" class="border-0 focus:ring-0">
                ...
                <option {{request('sale') == '2' ? 'selected="selected"' : ''}} value="2">{{ __('lms::Rent') }}</option>
                <option {{request('sale') == '1' ? 'selected="selected"' : ''}} value="1">{{ __('lms::Sale') }}</option>
            </select>
        </div>
        ...
        <div class="flex flex-col mx-3">
            ...
            <select id="type" name="type" class="border-0 focus:ring-0">
                ...
                <option {{request('type') == '3' ? 'selected="selected"' : ''}} value="3">{{ __('lms::Land') }}</option>
                <option {{request('type') == '1' ? 'selected="selected"' : ''}} value="1">{{ __('lms::Apartment') }}</option>
                <option {{request('type') == '2' ? 'selected="selected"' : ''}} value="2">{{ __('lms::Villa') }}</option>
            </select>
        </div>
        <div class="py-3 self-center border-gray-500 border"></div>
        <div class="flex flex-col mx-3">
            ...
            <select id="price" name="price" class="border-0 focus:ring-0">
                ...
                <option {{request('price') == '200000' ? 'selected="selected"' : ''}} value="200000">100000 - 200000 {{ __('lms::Tk') }}</option>
                <option {{request('price') == '300000' ? 'selected="selected"' : ''}} value="300000">200000 - 300000 {{ __('lms::Tk') }}</option>
                <option {{request('price') == '400000' ? 'selected="selected"' : ''}} value="400000">300000 - 400000 {{ __('lms::Tk') }}</option>
                <option {{request('price') == '500000' ? 'selected="selected"' : ''}} value="500000">400000 - 500000 {{ __('lms::Tk') }}</option>
                <option {{request('price') == '500000+' ? 'selected="selected"' : ''}} value="500000+">500000 up {{ __('lms::Tk') }}</option>
            </select>
        </div>
        <div class="py-3 self-center border-gray-500 border"></div>
        <div class="flex flex-col mx-3">
    ...
    <div class="flex justify-between items-end flex-1 ml-5">
      <input name="property_name" value="{{request('property_name')}}" type="search" placeholder="{{ __('lms::Search here') }}" class="rounded-lg px-2 py-2 w-full mr-4 focus:border-gray-700 focus:ring-0">
