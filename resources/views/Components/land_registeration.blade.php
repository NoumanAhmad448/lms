    ...
    <div class="lg:col-span-2">
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
            ...
            @include(config('files.forms') . 'col', [
                'input' => config('files.forms') . 'field',
                config('vars.prop') => [
                    config('vars.id') => config('setting.title'),
                    config('vars.label') => __('lms::messages.land_title'),
            ...
            @include(config('files.forms') . 'col', [
                'input' => config('files.forms') . 'field',
                config('vars.prop') => [
                    config('vars.id') => config('setting.description'),
                    config('vars.label') => __('lms::messages.land_des'),
            ...
            @include(config('files.forms') . 'col', [
                'input' => config('files.forms') . 'field',
                config('vars.prop') => [
                    config('vars.id') => config('setting.location'),
                    config('vars.label') => __('lms::messages.location_desc'),
            ...
            @include(config('files.forms') . 'col', [
                'input' => config('files.forms') . 'field',
                config('vars.prop') => [
                    config('vars.id') => config('setting.size'),
                    config('vars.label') => __('lms::messages.size_desc'),
            ...
            @include(config('files.forms') . 'col', [
                ...
                'col' => 3,
                config('vars.prop') => [
                    config('vars.id') => config('setting.city'),
                    config('vars.label') => __('lms::messages.city_desc'),
            ...
            @if (config('setting.en_country'))
                @include(config('files.forms') . 'col', [
                    ...
                    config('vars.prop') => [
                        'col' => 2,
                        config('vars.id') => config('setting.country'),
                        config('vars.label') => __('lms::messages.country_desc'),
                    ],
                ])
            @endif
            @if (config('setting.en_land_reg_file'))
                @include(config('files.forms') . 'col', [
                    'input' => config('files.forms') . 'file',
                    config('vars.prop') => [
                        'col' => 2,
                        config('vars.id') => config('setting.land_reg_file_upload'),
                        'include_star' => false,
                        config('vars.label') => __('lms::messages.file_upload_desc'),
