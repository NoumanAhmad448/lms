...
<div class="grid gap-4 gap-y-2 text-sm grid-cols-1 lg:grid-cols-3 my-4">
    ...
    <div class="lg:col-span-2">
        <div class="grid gap-4 gap-y-2 text-sm grid-cols-1 md:grid-cols-5">
            ...
            config("vars.prop") => [config('vars.id') => config("setting.title"), config('vars.label') => __("lms::messages.land_title"),
            config('vars.value') => $land ? $land?->title : ''
            ]])
            @include(config("files.forms").'col', ['input' => config("files.forms").'field',
            config("vars.prop") => [config('vars.id') => config("setting.description"), config('vars.label') => __("lms::messages.land_des"),
            config('vars.value') => $land ? $land?->description : ''
            ]])
            @include(config("files.forms").'col', ['input' => config("files.forms").'field',
            config("vars.prop") => [config('vars.id') => config("setting.location"), config('vars.label') => __("lms::messages.location_desc"),
            config('vars.value') => $land ? $land?->location : ''

            ]])
            @include(config("files.forms").'col', ['input' => config("files.forms").'field',
            config("vars.prop") => [config('vars.id') => config("setting.size"), config('vars.label') => __("lms::messages.size_desc"),
            config('vars.value') => $land ? $land?->size : ''
            ]])
            @include(config("files.forms").'col', ['input' => config("files.forms").'dropdown',
            "col" => 3 , config("vars.prop") => [config('vars.id') => config("setting.city"), config('vars.label') => __("lms::messages.city_desc"),
            ...
            @if(config("setting.en_land_reg_file"))
                @include(config("files.forms").'col', ['input' => config("files.forms").'file',
                 ...
                 "include_star" => false, config('vars.label') => __("lms::messages.file_upload_desc"),
