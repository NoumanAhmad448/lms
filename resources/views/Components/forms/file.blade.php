@php
    ...
    $message = $prop['message'] ?? __('lms::messages.file_upload_msg');
    $input_title = $prop['input_title'] ?? __('lms::messages.file_upload_title');
...
<div class="flex items-center justify-center w-full">
    <label for="{{ $id }}"
        ...
        <input name="{{ $id }}@if ($is_multiple) [] @endif" id="{{ $id }}"
            type="file" class="hidden" onchange="showImage(this, 'image{{ $id }}')"
...
<section class="image{{ $id }} flex items-center justify-center mt-5">
    @if ($files)
        @foreach ($files as $file)
            <img id='img{{ $file?->id }}' src='{{ file_path($file->link) }}' width="150" height="150"
                class="pr-4 rounded-lg
