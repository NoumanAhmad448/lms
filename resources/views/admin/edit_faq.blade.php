...
@section('content')
    ...
    <form method="POST" action="{{ route('admin_update_faq', compact('faq')) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <textarea class="form-control mb-2" rows="15" id="message" name="message" placeholder="Message">{{ $faq->message ?? '' }}</textarea>
            @error('message')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <img src="{{ config('setting.s3Url') . $faq->upload_img }}" alt="lms" width="100" height="50"
