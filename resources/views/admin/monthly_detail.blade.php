...
@section('content')
    ...
    <div class="d-flex justify-content-end my-3">
        <a href="{{ route('total-earning-detail', ['id' => $user->id]) }}" class="btn btn-website btn-lg"> Total Earning
            Details</a>
    </div>
    ...
    <section class="d-flex justify-content-between align-items-center">
        ...
        <div class="badge badge-success" style="font-size: 1rem"> Current Month Earning ${{ $current_month_earning ?? 0 }}
        </div>
                        ...
                        <tbody>
                            @foreach ($payment_history as $month => $earn)
                                <tr>
                                    ...
                                    <td
                                        class="@if (!$p_u) {{ __('lms::bg-success') }} @else {{ __('lms::bg-danger') }} @endif">
                                        {{ $p_u ? 'unpaid' : 'paid' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
