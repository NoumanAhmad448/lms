...
@section('content')
    ...
    <div class="container">
        <div class="row">
            ...
            <div class="col-md-2">
                <button type="button" class="btn btn-info my-1 my-md-0" id="update"> <i class="fa fa-floppy-o"
                ...
                <form action="{{ route('a_c_sorting') }}" method="POST">
                    ...
                    <div class="form-group">
                        <select class="custom-select  mb-3" id="sorting" name="sorting">
                            ...
                            <option value="ai" @if ($order === 'ai') {{ __('selected') }} @endif> Ascending
                                by ID</option>
                            <option value="di" @if ($order === 'di') {{ __('selected') }} @endif> Desc by ID
                            </option>
                            <option value="at" @if ($order === 'at') {{ __('selected') }} @endif> Ascending
                                by title </option>
                            <option value="dt" @if ($order === 'dt') {{ __('selected') }} @endif> Desc by
                                title </option>
                            <option value="ac" @if ($order === 'ac') {{ __('selected') }} @endif> Ascending
                                by categories </option>
                            <option value="dc" @if ($order === 'dc') {{ __('selected') }} @endif> Desc by
                                categories </option>
                        </select>
                    </div>
                </form>
            ...
            <div class="col-md-6">
                <form action="{{ route('a_c_searching') }}" method="POST">
                    ...
                    <div class="input-group mb-3">
                        <input type="text" class="form-control" placeholder="Search by course name,category"
                            id="search_item" name="search_item"
                            value="@if ($res) {{ $res }} @endif">
    ...
    @if ($courses->count())
                ...
                <tbody>
                    @foreach ($courses as $course)
                        <tr>
                            <td>
                                <div class="form-check">
                                    <input type="checkbox" class="form-check-input course_no" name="course_no"
                                        values="{{ $course->id }}" />
                                </div>
                            </td>
                            <td> {{ $course->id ?? '' }}</td>
                        ...
                        @elseif($s == 'pending') {{ __('badge-warning') }} @elseif($s == 'published') {{ __('badge-success') }} @elseif($s == 'draft') {{ __('badge-danger') }} @endif">
                            ...
                            <td>
                                <div href="{{ route('course_delete', ['course_id' => $course->id]) }}"
                                    class="text-danger cursor_pointer c_del_by_a"> Delete </div>
                            </td>
                        </tr>
        ...
        <div class="text-md-right text-center my-5 mb-md-0">
            {{-- {{ $courses->links() }} --}}
        </div>
    @else
...
@section('page-js')
    {{-- <script src="{{asset('vendor/lms/js/admin_courses.js')}}">       --}}
    {{-- </script> --}}
    <script>
        $(function() {
            ...
            $('#update').click(function() {
                ...
                let url = "{{ route('change_course_status') }}";
                console.log(url);
                if (status !== "") {
