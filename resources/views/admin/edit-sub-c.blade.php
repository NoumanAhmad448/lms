...
@section('content')
    ...
    <form action="{{ route('admin_update_sub_c', compact('c')) }}" method="post" class="mt-2">
        ...
        <div class="form-group">
            ...
            <select class="custom-select  mb-1 @error('sub_c') is-invalid @enderror" id="sub_c" name="sub_c">
                ...
                @foreach ($categories as $ca)
                    <option value="{{ $ca->value ?? null }}"
                        @if ($c->categories_id == $ca->id) {{ __('lms::selected') }} @endif> {{ $ca->name ?? 'DB ERROR' }}
                    </option>
                @endforeach
            </select>
            @error('sub_c')
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control mb-2 @error('name') is-invalid @enderror" id="name"
                aria-describedby="category name" name="name" placeholder="Category Name" value="{{ $ca->name ?? '' }}">
            @error('name')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <button type="submit" class="btn btn-lg website-outline"> <i class="fa fa-floppy-o" aria-hidden="true"></i> Update
            SubCategory </button>
    </form>
@endsection
