                    ...
                    <div class="form-group d-flex align-items-center">
                        ...
                        <select class="custom-select custom-select mb-3" id="sorting" name="sorting">
                            ...
                            <option value="di" @if ($order === 'di') {{ __('lms::selected') }} @endif>
                                Descending by ID</option>
                            <option value="an" @if ($order === 'an') {{ __('lms::selected') }} @endif>
                                Ascending by Name</option>
                            <option value="dn" @if ($order === 'dn') {{ __('lms::selected') }} @endif>
                                Descending by Name</option>
                            <option value="ae" @if ($order === 'ae') {{ __('lms::selected') }} @endif>
                                Ascending by Email</option>
                            <option value="de" @if ($order === 'de') {{ __('lms::selected') }} @endif>
                                Descending by Email</option>
                        </select>
                    </div>
                </form>
            </div>
                ...
                <form action="{{ route('search_users') }}" method="post">
                    ...
                    <div class="input-group mb-3">
                        <input type="text" class="form-control" placeholder="Search by name or email"
                            value="@if ($search_item) {{ $search_item }} @endif" id="search_item"
                            name="search_item">
                        <div class="input-group-append">
                            <button type="submit" class="btn btn-info" id="search"> <i class="fa fa-search"
                                    aria-hidden="true"></i>
                                Search
                            </button>
                        </div>
...
@section('page-js')
    <script src="{{ asset('vendor/lms/js/users.js') }}"></script>
@endsection
