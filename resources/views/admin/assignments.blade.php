                ...
                <div class="form-group">
                    <select class="custom-select custom-select mb-3" id="sorting" name="sorting">
                        ...
                        <option value="ai" @if ($order === 'ai') {{ __('lms::selected') }} @endif> Ascending
                            by ID</option>
                        <option value="di" @if ($order === 'di') {{ __('lms::selected') }} @endif> Desc by
                            ID </option>
                        <option value="an" @if ($order === 'an') {{ __('lms::selected') }} @endif> Ascending
                            by name </option>
                        <option value="dn" @if ($order === 'dn') {{ __('lms::selected') }} @endif> Desc by
                            name </option>
                    </select>
        ...
        <div class="col-md-6">
            <form action="{{ route('a_a_searching') }}" class="post" method="POST">
                ...
                <div class="input-group mb-3">
                    <input type="text" class="form-control" placeholder="Search by course name" id="search_item"
                        name="search_item">
