    ...
    <form action="{{ route('admin_store_sub_c') }}" method="post">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control mb-2 @error('name') is-invalid @enderror" id="name"
                aria-describedby="category name" name="name" placeholder="Category Name" value="{{ old('name') }}">
            @error('name')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>

        <button type="submit" class="btn website-outline btn-lg"> <i class="fa fa-floppy-o" aria-hidden="true"></i> Save
        </button>
    </form>
@endsection
