...
@section('content')
    <h1> Total number of Enrollments </h1>
    <span class="badge badge-success"> Total {{ __('lms::coupon.Enrollment') }}
        {{ $total_enrollment }}
    </span>
    @if (config('setting.admin_guest_search_bar'))
        <div class="col-md-5 my-3" style="min-width: 100%">
            <div class="searchbar mt-4 mt-md-0">
                ...
                <div class="search_icon btn"><i class="fa fa-search" aria-hidden="true"></i>
                </div>
            </div>
        </div>
    @endif
                            ...
                            @if (config('setting.enable_course_unenroll'))
                                <td>
                                    <div class="btn btn-info"
                                        onclick="unenroll_user( @if ($course->user->id && $course->id) {!! $course->user->id !!},{!! $course_id !!} @endif)">
                                        Unenroll </div>
                                </td>
                            @endif
                        </tr>
