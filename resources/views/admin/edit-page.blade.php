    ...
    <form method="POST" action="{{ route('admin_update_page', compact('page')) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control mb-1 @error('title') is-invalid @enderror" id="title"
                name="title" placeholder="Title" value="{{ $page->title ?? '' }}">
            @error('title')
        ...
        <div class="form-group">
            ...
            <textarea class="form-control mb-2" rows="15" id="message" name="message" placeholder="Message">{{ $page->message ?? '' }}</textarea>
            @error('message')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
        <img src="{{ config('setting.s3Url') . $page->upload_img }}" alt="lms" width="100" height="50"
        ...
        <button type="submit" class="btn btn-lg btn-info">
            <i class="fa fa-floppy-o" aria-hidden="true"></i> Update Page
        </button>
    </form>
@endsection
