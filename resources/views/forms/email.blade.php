...
@if ($is_form)
    <label for="{{ config('form.email') }}">
        {{ __('lms::messages.EmailAddress') }}@if ($include_star)
            {!! config('setting.red_star') !!}
        @endif
    </label>
    <input type="email" name="{{ config('form.email') }}" id="{{ config('form.email') }}"
        placeholder="{{ __('lms::label.EmailAddressSample') }}" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50"
        value="{{ $email ?? '' }}" />
@else
    <label for="{{ config('form.email') }}" class="mb-2.5 block font-medium text-black dark:text-white">
        {{ __('lms::label.EmailAddress') }}</label>
    <div class="relative">
        <input type="email" placeholder="{{ __('lms::label.EmailAddressSample') }}" name="{{ config('form.email') }}"
