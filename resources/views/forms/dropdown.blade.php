@php
    use Eren\Lms\Models\Country;
    use Eren\Lms\Models\City;
    use Eren\Lms\Models\Job;
    ...
    <div id="{{ $id }}dropdown" autocomplete="off"
        ...
        @if ($data)
            @foreach ($data as $record)
                <p class="dropdown-item block px-4 py-2 text-gray-700 hover:bg-gray-100 active:bg-blue-100 cursor-pointer rounded-md"
                    value="
                    {{-- treat $record as an array --}}
                    @if (is_key_exists(config('vars.id'), $record) && $record[config('vars.id')]) {{ $record[config('vars.id')] }}
