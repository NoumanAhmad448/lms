<div>
    ...
    <x-jet-form-section submit="createApiToken">
        <x-slot name="title">
            {{ __('lms::Create API Token') }}
        </x-slot>

        <x-slot name="description">
            {{ __('lms::API tokens allow third-party services to authenticate with our application on your behalf.') }}
        </x-slot>

        <x-slot name="form">
            <!-- Token Name -->
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="name" value="{{ __('lms::Token Name') }}" />
            ...
            @if (Laravel\Jetstream\Jetstream::hasPermissions())
                <div class="col-span-6">
                    <x-jet-label for="permissions" value="{{ __('lms::Permissions') }}" />
    ...
    @if ($this->user->tokens->isNotEmpty())
        ...
        <div class="mt-10 sm:mt-0">
            <x-jet-action-section>
                <x-slot name="title">
                    {{ __('lms::Manage API Tokens') }}
                </x-slot>

                <x-slot name="description">
                    {{ __('lms::You may delete any of your existing tokens if they are no longer needed.') }}
                </x-slot>
                                ...
                                <div class="flex items-center">
                                    @if ($token->last_used_at)
                                        <div class="text-sm text-gray-400">
                                            {{ __('lms::Last used') }} {{ $token->last_used_at->diffForHumans() }}
                                        </div>
                                    @endif
    ...
    <x-jet-dialog-modal wire:model="displayingToken">
        ...
        <x-slot name="content">
            <div>
                {{ __('lms::Please copy your new API token. For your security, it won\'t be shown again.') }}
            </div>
    ...
    <x-jet-confirmation-modal wire:model="confirmingApiTokenDeletion">
        <x-slot name="title">
            {{ __('lms::Delete API Token') }}
        </x-slot>

        <x-slot name="content">
            {{ __('lms::Are you sure you would like to delete this API token?') }}
        </x-slot>
