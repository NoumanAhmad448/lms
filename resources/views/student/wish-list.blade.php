                    ...
                    <div class="col-md-3 my-5">
                        @if ($course->slug)
                            <div class="card" style="width: 18rem;">
                                ...
                                <div class="card-body">
                                    <h5 class="card-title font-bold text-capitalize"> {{ $course->course_title ?? '' }}
                                    </h5>
                                    <p class="card-text text-capitalize mt-1"> By <span class="font-bold">
                                            {{ $course->user->name ?? '' }} </span> </p>
                                    <p class="card-text text-capitalize mt-1"> Category
                                        {{ $course->categories_selection ?? '' }} </p>
                                ...
                                <form action="{{ route('remove-wishlist-course', ['slug' => $course->slug]) }}"
                                    method="post">
                                    @csrf
                                    @method('delete')
